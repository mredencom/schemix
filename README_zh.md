...
| 操作 | 耗时 | 内存 | 分配次数 |
|------|------|------|----------|
| `New`（编译） | 437 µs | 811 KiB | 22423 |
| `Process`（合法） | **5.15 µs** | 11.81 KiB | 81 |
| `Process`（非法） | 6.67 µs | 13.14 KiB | 97 |
| `Process`（嵌套） | 31.59 µs | 42.07 KiB | 417 |
| `Validate`（无输出） | **5.08 µs** | 11.46 KiB | 77 |
| `Process`（并行，10 核） | **4.91 µs** | 11.81 KiB | 81 |
| `ValidateFields`（快速路径） | 162.9 ns | 0 B | 0 |
| `Validate`（3 个标量列表，各 1 元素） | **81.1 ns** | 0 B | 0 |
| `Validate`（3 个标量列表，各 10 元素） | **350 ns** | 0 B | 0 |
| `Registry.Get` | 6.37 ns | 0 B | 0 |

> 简单标量字段使用 Go 原生快速路径，完全绕过 CUE，
> 相比 CUE 旧路径实现约 **157 倍加速**（162.9ns vs 25.62µs）。
>
> `cue.Context.Encode` 现在是**惰性**的：如果 schema 的所有字段都由快速路径处理，
> 输入数据根本不会被转换成 `cue.Value`。上表每一行相比早期版本少掉的正好是那 44 次分配
> （`Process` 125 → 81，`Validate` 121 → 77）。含结构体的 schema 仍然需要该转换，
> 这正是 `Process`（嵌套）测量的内容 —— 由于字段查找路径改为编译期构建而非每次调用
> 重新解析，分配次数从 492 降到 417。
>
